...
Classical few-shot segmentation aggregates dense correspondences between a support image-mask pair and a query image, with a sequence of architectures progressively refining the aggregation strategy: prior-guided enrichment @tian2020pfenet, hypercorrelation @min2021hsnet, base/meta separation @lang2022bam, 4D cost aggregation @hong2022vat, and dense cross-attention @shi2022dcama @peng2023hdmnet. These methods reach strong mIoU on FSS-1000 but require training on the few-shot task itself and assume the query mask can be regressed densely.

SAM @kirillov2023sam and SAM2 @ravi2024sam2 made one-shot segmentation without tractable from a small number of point or box prompts without per-class fine-tuning. The SAM model consists of an: 1.) image encoder, 2.) prompt decoder, and 3.) mask decoder. The model takes point prompts as position coordinates $(x,y)$ and a positive/negative $(0/1)$ flag for each point. Subsequent work pairs SAM with feature-matching for training-free few-shot segmentation @liu2024matcher @zhang2023persam, conditions a single model on an in-context example @wang2023seggpt, or adapts SAM2 with a trained adapter @torres2025sansa. Liu et al. recast one-shot segmentation as dense feature matching on top of SAM embeddings, enabling class-agnostic mask transfer from a single annotated example without additional training @liu2024matcher. Zhang et al. introduce a lightweight test-time personalization of SAM by adapting its features using a single reference image, allowing it to specialize to a new concept with minimal optimization @zhang2023persam. Wang et al. frame segmentation as an in-context learning problem using a GPT-style architecture that predicts masks conditioned on example image–mask pairs, eliminating task-specific training @wang2023seggpt. Torres et al. add a parameter-efficient adapter to SAM2 that enables single-image few-shot semantic segmentation by injecting task-specific information without retraining the full model @torres2025sansa. These methods vary widely in how much information they pass to the segmentation backbone, leaving the prompt-selection rule itself underexplored.

A separate line of work trains networks to consume simulated points for interactive segmentation @sofiiuk2022ritm @liu2023simpleclick @chen2022focalclick. These methods operate in a fundamentally different setting from ours because the user (or simulator) provides the point sequence; the network is trained to consume points rather than produce them. To our knowledge, the present work is the first systematic comparison of heuristic, imitation, and reward-driven point selection on a unified action space against a frozen SAM decoder.
...
In addition to the eleven heuristics, we evaluate a learned policy trained by behavioral cloning (BC) on a greedy oracle. For each training episode, we discretize the $224 times 224$ query grid to a $37 times 37$ candidate grid and, for each candidate position $(x_c, y_c)$, query SAM under the prompt set $cal(P)_t union {(x_c, y_c)}$ and record the resulting IoU against the ground-truth mask. The oracle's $t$-th point is the candidate that maximizes this IoU. Because the oracle's first $k$ points are identical for any $N >= k$ by construction, a single $N = 10$ trajectory yields oracle labels for every smaller budget, and we cache trajectories per episode to memory so that the imitator never re-invokes SAM during training. On the FSS-1000 validation split this oracle reaches a mIoU $0.948$ at $N = 2$, $0.952$ at $N = 3$, $0.956$ at $N = 5$, and $0.955$ at $N = 10$, confirming that the $37 times 37$ grid resolution is not the bottleneck and that substantial headroom exists above the heuristics.
 ...
 The loss is a Gaussian-kernel-density formulation of cross-entropy in which the oracle target is rendered as a Gaussian heatmap with $sigma = 2$ grid cells, so that the gradient signal degrades gracefully near the correct cell rather than treating any non-exact prediction as equally wrong. We add an auxiliary IoU loss term that, on a sub-batch of training examples, decodes the policy's argmax point and computes IoU against the ground-truth mask, providing a small, IoU-aligned supervisory signal. To match the inference-time setting, where adding points to a cell that has points is meaningless, we remove cells with points out of the policy's logit space at training time. We train one policy per budget $N in {2, 5, 10}$ on the FSS-1000 train classes, select checkpoints by full-validation rollout IoU on the $450$ validation episodes, and evaluate on the held-out test split using the same eval setup as the heuristic baselines.
...
We use standard PPO hyperparameters, with clip $epsilon = 0.2$, GAE $lambda = 0.95$, $gamma = 1.0$, four update epochs per rollout, and an effective rollout batch of $32$ vectorized environments times episode length. Across one million environment steps, the KL anchor coefficient $beta$ decays linearly from $0.1$ to $0.01$, the entropy coefficient from $0.01$ to $0.001$, and the learning rate from $3 times 10^(-4)$ to zero. Early steps keep the policy ($pi_theta$) close to the BC reference while late steps relax the constraints and let the agent optimize the change in IoU directly. SAM ViT-B is used during PPO training to reduce per-call cost; final test evaluation uses the same SAM ViT-H decoder used by every other method, keeping the comparison unbiased.
...
The visual contrast confirms the results of @tab:test_results: the RL fine-tune is consistently competitive, BC has individual-episode failure modes at long horizons, and heuristics depend heavily on the scene.
...
The behavioral-cloning policy provides a more interesting case. The greedy oracle that BC imitates is itself SAM-aware, in the sense that each oracle point is selected by maximizing IoU under SAM. Yet a policy trained to clone the oracle's points does not match the oracle's per-step trajectory. The cloned policy reproduces a pattern of plateau and decline rather than the oracle's continued ascent, peaking at point step $3$ at $N = 10$ and degrading thereafter. Imitation alone, even of a strong oracle, appears to inherit the heuristic-style ceiling rather than break through it.

The plateau-and-decline that bounds the heuristic family and the imitation policy is broken cleanly by reward-driven training. The operative difference is the alignment of the optimization signal. Imitation forces the policy to commit to the specific cells the oracle chose at each step, which is brittle on the test distribution; reward-driven training lets the policy choose any cell whose point happens to improve SAM's mask, and over many gradient steps that flexibility lets the policy discover point sequences whose joint information content exceeds anything any single oracle trajectory exhibits.
