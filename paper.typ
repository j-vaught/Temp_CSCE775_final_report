...
SAM @kirillov2023sam and SAM2 @ravi2024sam2 made one-shot segmentation tractable from a small number of point or box prompts without per-class fine-tuning. Subsequent work pairs SAM with feature-matching for training-free few-shot segmentation @liu2024matcher @zhang2023persam, conditions a single model on an in-context example @wang2023seggpt, or adapts SAM2 with a trained adapter @torres2025sansa. These methods vary widely in how much information they pass to the segmentation backbone, leaving the prompt-selection rule itself underexplored.

A separate line of work trains networks to consume simulated point-clicks for interactive segmentation @sofiiuk2022ritm @liu2023simpleclick @chen2022focalclick. These methods operate in a fundamentally different setting from ours because the user (or simulator) provides the click sequence; the network is trained to consume points rather than produce them. To our knowledge, the present work is the first systematic comparison of heuristic, imitation, and reward-driven point selection on a unified action space against a frozen SAM decoder.
